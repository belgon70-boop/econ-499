Project Progress Update: Section 321 De Minimis Trade & Fentanyl Supply
Author: Belén Gonzalez
...
Initial Objective
To build a balanced state-level panel dataset (2018–2025) combining:
Supply Proxies: State-level and Port-of-Entry de minimis package counts, Entry Type 86 filings, and International Mail Facility (IMF) drug seizures.
Public Health Outcomes: CDC WONDER age-adjusted synthetic opioid mortality rates and state overdose counts.
...
This is a final response to your Freedom of Information Act (FOIA) request to U.S. Customs and Border Protection (CBP), requesting de-minimus and seizure data.

CBP is granting your request under the FOIA, Title 5 U.S.C. § 552. Upon initial review of your request, we have determined that the following documents can be found on the internet at the following links:
...
Proposed Next Steps & Methodological Pivots
Given the administrative limitations of CBP microdata, I propose to look into using secondary datasets and structural policy breaks:
1. Policy Shock Design
Evaluate the impact of major federal policy shifts regarding de minimus trade on national drug trends
...
3. Logistics Gateway & Infrastructure Proxies
Since direct port-level package counts are restricted, we can proxy state-level package exposure using:
